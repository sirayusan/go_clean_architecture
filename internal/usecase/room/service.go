...
func (uc *MessageUseCase) PubSub(c echo.Context, ws *websocket.Conn, roomManager map[uint32]*entity.ChatRoom, chatRoomID uint32, rdb *redis.Client) {
	// chatRoomID を文字列に変換
	channelName := "room-" + fmt.Sprint(chatRoomID)

	// 新しいgoroutineを起動
	go func() {
		defer ws.Close() // goroutineが終了するときにWebSocket接続を閉じる
...
func (uc *MessageUseCase) processMessage(c echo.Context, roomManager map[uint32]*entity.ChatRoom, rdb *redis.Client, channelName string, chatRoomID uint32, _msg string) error {
