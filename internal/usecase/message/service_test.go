...
func (m *MockChatRepo) GetMessageList(userID uint32) (entity.Messages, error) {
	args := m.Called(userID)
	return args.Get(0).(entity.Messages), args.Error(1)
}
...
func TestAuthentication(t *testing.T) {
	...
	chats, err := uc.GetMessages(uint32(1))
	...
	chats, err = uc.GetMessages(uint32(0))
	assert.Error(t, err)
	...
	chats, err = uc.GetMessages(uint32(2))
	assert.Error(t, err)
	assert.Empty(t, chats)
